...
function getData(gitHubUser) {
    let rezultat = new Promise(function (resolve, reject) {
        $.ajax({ 
            ...
            reject('Error: An error occurred. ' + JSON.stringify(status) + JSON.stringify(errorThrown))
                ...
                <div class="service-box">
                    <div class="service-ico">
                        <a href='${item.html_url}' target='_blank'>
                            <img style="max-width:100%; display:block;" src ="https://raw.githubusercontent.com/${item.owner.login}/${item.name}/${item.default_branch}/banner.png">
                        </a>
                    </div>
...
$(function () {
    console.log("ready!");
    getData('mariaadelam').then((myRepos) => {
        showData(myRepos);
